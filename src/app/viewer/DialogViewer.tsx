    ...
    <Dialog.Root open={Boolean(imageDialog)} onOpenChange={() => setImageDialog(null)}>
      <Dialog.Portal className='min-h-max w-3/4'>
        <div className='fixed inset-0 z-10 flex items-start justify-center sm:items-center'>
          ...
          <Dialog.Content className='fixed flex items-center justify-center z-10 w-5/6  gap-4 rounded-b-lg border bg-background p-6 shadow-lg animate-in data-[state=open]:fade-in-90 data-[state=open]:slide-in-from-bottom-10 data-[state=open]:sm:slide-in-from-bottom-0'>
